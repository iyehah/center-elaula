...
class TeacherTab(ctk.CTkFrame):
    def __init__(self, root):
        ...
        self.conn = sqlite3.connect("./db/teacher_school.db")
    ...
    def display_teachers(self, filter_name="", filter_class="Tous"):
        ...
        for row, teacher in enumerate(teachers, start=1):  # Commencer à la ligne 1 pour laisser de l'espace pour les en-têtes
            ...
            # Bouton Modifier
            edit_button = ctk.CTkButton(actions_frame,fg_color="#4285F4", text="Modifier", width=50, command=lambda tid=teacher_id: self.edit_teacher(tid))
            edit_button.pack(side="left", expand=True, padx=5, pady=5)

            # Bouton Supprimer
            delete_button = ctk.CTkButton(actions_frame,fg_color="#4285F4", text="Supprimer", width=60, command=lambda tid=teacher_id: self.delete_teacher(tid))
